...
export const metadata: Metadata = {
  title: "Sponsors | Finance 2045 Jakarta, 7–8 July 2026",
  description: "Confirmed sponsors of Finance 2045 — Southeast Asia's preeminent financial summit.",
};
...
async function getSponsors(): Promise<{ categories: { name: string; items: Entity[] }[] }> {
  try {
    const res = await fetch("https://api.konfhub.com/event/public/wais-f45-indonesia/entity/1", {
      next: { revalidate: 300 },
    });
...
export default async function SponsorsPage() {
  const { categories } = await getSponsors();
    ...
    <>
      ...
      <div className="pt-page">
        <div className="pt-hero">
          ...
          <p>
            {total > 0
              ? `${total} confirmed sponsor${total > 1 ? "s" : ""} powering Finance 2045, Jakarta, 7–8 July 2026.`
              : "Sponsor announcements coming soon. Check back shortly."}
          </p>
        </div>

        <div className="pt-body">
          {categories.length === 0 ? (
            <div className="pt-empty">Sponsor announcements coming soon.</div>
          ) : (
            categories.map((cat) => (
              <div key={cat.name}>
