...
export const metadata: Metadata = {
  title: "Association Partners | Finance 2045 Jakarta, 7–8 July 2026",
  description: "Official association partners of Finance 2045 — Southeast Asia's preeminent financial summit.",
};
...
async function getAssociations(): Promise<{ categories: { name: string; items: Entity[] }[] }> {
  try {
    ...
    const f45Assoc = data.categorized
      .filter(
        (c) =>
          c.category_name.startsWith("Finance 2045") &&
          c.category_name.toLowerCase().includes("association")
      )
...
export default async function AssociationsPage() {
  const { categories } = await getAssociations();
    ...
    <>
      ...
      <div className="pt-page">
        <div className="pt-hero">
          ...
          <h1>Association <span>Partners</span></h1>
          <p>
            {total > 0
              ? `${total} confirmed association partner${total > 1 ? "s" : ""} supporting Finance 2045, Jakarta, 7–8 July 2026.`
              : "Association partner announcements coming soon."}
          </p>
        </div>

        <div className="pt-body">
          {categories.length === 0 ? (
            <div className="pt-empty">Association partner announcements coming soon.</div>
          ) : (
            categories.map((cat) => (
              <div key={cat.name}>
