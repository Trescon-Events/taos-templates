...
export const metadata: Metadata = {
  title: "Exhibitors | Finance 2045 Jakarta, 7–8 July 2026",
  description: "Confirmed exhibitors at Finance 2045 — Southeast Asia's preeminent financial summit.",
};
...
async function getExhibitors(): Promise<{ categories: { name: string; items: Entity[] }[]; uncategorized: Entity[] }> {
  try {
    const res = await fetch("https://api.konfhub.com/event/public/wais-f45-indonesia/entity/3", {
      next: { revalidate: 300 },
    });
    if (!res.ok) return { categories: [], uncategorized: [] };
    const data: { categorized: Category[]; uncategorized: Entity[] } = await res.json();

    const f45 = data.categorized
      .filter((c) => c.category_name.startsWith("Finance 2045"))
    ...
    return { categories: f45, uncategorized: data.uncategorized ?? [] };
  } catch {
...
export default async function ExhibitorsPage() {
  const { categories, uncategorized } = await getExhibitors();
  const total = categories.reduce((s, c) => s + c.items.length, 0) + uncategorized.length;
    ...
    <>
      ...
      <div className="pt-page">
        <div className="pt-hero">
          ...
          <h1>Our <span>Exhibitors</span></h1>
          <p>
            {total > 0
              ? `${total} confirmed exhibitor${total > 1 ? "s" : ""} showcasing at Finance 2045, Jakarta, 7–8 July 2026.`
              : "Exhibitor announcements coming soon."}
          </p>
        </div>

        <div className="pt-body">
          {categories.length === 0 && uncategorized.length === 0 ? (
            <div className="pt-empty">Exhibitor announcements coming soon.</div>
          ) : (
            <>
              {categories.map((cat) => (
                <div key={cat.name}>
                  ...
                  <div className="pt-grid">
                    {cat.items.map((e) => (
                      <a key={e.id} href={e.website_url ?? "#"} target="_blank" rel="noopener noreferrer" className="pt-card">
              ...
              {uncategorized.length > 0 && (
                <div>
                  <div className="pt-cat-label">Exhibitors</div>
                  <div className="pt-grid">
                    {uncategorized.map((e) => (
                      <a key={e.id} href={e.website_url ?? "#"} target="_blank" rel="noopener noreferrer" className="pt-card">
                        <div className="pt-logo">
